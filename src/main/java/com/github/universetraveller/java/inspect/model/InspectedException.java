...
public class InspectedException extends InspectedEvent {
    ...
    public ObjectReference getExceptionReference() {
    ...
    public static InspectedException getInstance(Inspector inspector, ExceptionEvent event) {
        ...
        try {
            ...
            int depth = inspector.getMaxFrameCountToInspect() > event.thread().frameCount()
                    ? event.thread().frameCount()
                    : inspector.getMaxFrameCountToInspect();
            instance.stackTrace = JDIReachingUtil.buildStackTrace(event.thread().frames(0, depth));
        } catch (AbsentInformationException e) {
            inspector.getLogger()
                    .warning(String.format("%s occurs at building stacktrace at handling exception event; skip", e));
        } catch (IncompatibleThreadStateException | VMDisconnectedException f) {
            inspector.getLogger()
                    .warning(String.format("%s occurs at building stacktrace at handling exception event; skip", f));
        } finally {
            if (lock)
    ...
    protected String internalBuildString() {
        ...
        builder.append(String.format("<Exception type='%s' occurAt='%s'>", this.exceptionReference, this.location))
                .append("\n");
